 ...
 *  
 *   1.javax.servlet.Filter 인터페이스 구현힌다.
 *   2.어떤 용청에 대해 필터링한것인지 맵핑한다.
 */

@WebFilter({"/private/*","/users/private/*","/cafe/private/*","/file/private/*"})/*private 하위에 모든 요청에 대해서 이다. [*] 사용가능 나열하고싶을때는 {}로 감싸고 ,로 나눠준다.*/
...
//실행순서를 중간에 가로채는 느낌~ webfilter의 경로안에 있는 모든것도 선택할수있고 
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
	      ...
	      if(id != null) {
	         ...
	         //리다일렉트 시킬때 원래 목적지 정보를 url 라는 파라미터 명으로 같이 보낸다. 어디로 보낼지?
	         res.sendRedirect(cPath+"/users/loginform.jsp?url="+encodedUrl);
	      }   
	}//request, respones를 받아야 흐름이 이어갈수있다. 
